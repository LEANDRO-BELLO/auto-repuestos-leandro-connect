...
function renderRows(items) {
  if (!items.length) {
    return `
      <tr>
        <td class="doc-empty" colspan="7">No hay servicios realizados en el período seleccionado.</td>
      </tr>
    `;
  }
...
export function buildRelatorioServiciosHtml({ empresa, items, fechaDesde, fechaHasta }) {
...
<head>
  <meta charset="UTF-8" />
  <title>REPORTE DE SERVICIOS REALIZADOS</title>
...
<body>
  <div class="doc">
    ...
    <div class="doc-title">REPORTE DE SERVICIOS REALIZADOS</div>
...
export async function exportRelatorioServiciosPdf({ fechaDesde, fechaHasta }) {
  ...
  const html = buildRelatorioServiciosHtml({ empresa, items, fechaDesde, fechaHasta });
  const agora = new Date();
  const pad = (valor) => String(valor).padStart(2, '0');
  const suggestedFilename = `reporte-servicios-realizados-${agora.getFullYear()}-${pad(agora.getMonth() + 1)}-${pad(agora.getDate())}-${pad(agora.getHours())}-${pad(agora.getMinutes())}-${pad(agora.getSeconds())}.pdf`;
