...
import { canonicalServicioId, resolveServicioLabel } from './servicios-labels.js';
...
function matchesServicio(servicioId, filtroServicio) {
  if (!filtroServicio || filtroServicio === 'todos') {
    return true;
  }

  return canonicalServicioId(servicioId) === canonicalServicioId(filtroServicio);
}

export function expandServiciosPorTipo(items, servicioId = 'todos') {
  const rows = [];

  for (const item of items || []) {
    const ids = Array.isArray(item.servicios) ? item.servicios : [];
    const labels = Array.isArray(item.serviciosLabels) ? item.serviciosLabels : [];

    ids.forEach((id, index) => {
      if (!matchesServicio(id, servicioId)) {
        return;
      }
      ...
      rows.push({
        ...
        servicioLabel: labels[index] || resolveServicioLabel(id)
      });
    });
  }
  ...
  return rows.sort((a, b) => {
    ...
    const osDiff = String(a.numeroOs || '').localeCompare(String(b.numeroOs || ''));
    if (osDiff !== 0) {
      return osDiff;
    }

    return String(a.servicioLabel || '').localeCompare(String(b.servicioLabel || ''), 'es', {
      sensitivity: 'base'
    });
  });
}
...
function renderRows(items) {
  if (!items.length) {
    return `
      <tr>
        <td class="doc-empty" colspan="7">No hay servicios en el período/filtro seleccionado.</td>
      </tr>
    `;
  }
...
export function buildRelatorioServiciosPorTipoHtml({ empresa, items, fechaDesde, fechaHasta }) {
...
<head>
  <meta charset="UTF-8" />
  <title>REPORTE DE SERVICIOS POR TIPO</title>
...
<body>
  <div class="doc">
    ...
    <div class="doc-title">REPORTE DE SERVICIOS POR TIPO</div>
    ...
    <footer class="doc-footer">
      Total de servicios encontrados: ${escapeHtml(String(items.length))}
    </footer>
  </div>
</body>
</html>`;
...
export async function exportRelatorioServiciosPorTipoPdf({ fechaDesde, fechaHasta, servicioId = 'todos' }) {
  const [resultado, empresa] = await Promise.all([
    window.api.listServiciosRealizados({ fechaDesde, fechaHasta }),
    window.api.getEmpresa()
  ]);

  const items = expandServiciosPorTipo(resultado.items || [], servicioId);
  const html = buildRelatorioServiciosPorTipoHtml({ empresa, items, fechaDesde, fechaHasta });
  const agora = new Date();
  const pad = (valor) => String(valor).padStart(2, '0');
  const suggestedFilename = `reporte-servicios-por-tipo-${agora.getFullYear()}-${pad(agora.getMonth() + 1)}-${pad(agora.getDate())}-${pad(agora.getHours())}-${pad(agora.getMinutes())}-${pad(agora.getSeconds())}.pdf`;
